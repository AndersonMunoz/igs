...
export const actualizarProducto = async (req, res) => {
  try {
    ...
    let sql = `UPDATE productos SET fecha_caducidad_producto='${fecha_caducidad_producto}',cantidad_peso_producto='${cantidad_peso_producto}',unidad_peso_producto='${unidad_peso_producto}',descripcion_producto='${descripcion_producto}',precio_producto='${precio_producto}' WHERE id_producto=${id}`;
    const [rows] = await pool.query(sql);
    if (rows.affectedRows > 0) {
      res
        .status(200)
        .json({ status: 200, message: "Se actualizo con exito el producto" });
    } else {
      res
        .status(401)
        .json({ status: 401, message: "No se actualizo el producto" });
    }
  } catch (e) {
